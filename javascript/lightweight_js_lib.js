...
//   options[type].timeZoneName = "short";
  if (type === "dig") {
...
var div = document.createElement("div"),
  prefix =
    ["moz", "webkit", "ms", "o"].filter(function (prefix) {
...
function mbStrimwidth(str, start, width, trimmarker) {
  ...
  var trimmakerWidth = mbStrwidth(trimmarker),
    i = start,
    l = str.length,
    trimmedLength = 0,
    trimmedStr = "";
  for (; i < l; i++) {
    var charCode = str.charCodeAt(i),
      c = str.charAt(i),
      charWidth = mbStrwidth(c),
      next = str.charAt(i + 1),
      nextWidth = mbStrwidth(next);
...
function urlParse(str) {
  var urlArray = [];
  var urlRegex =
    /((http[s]?|ftp):\/)\/?([^:\/\s]+)(?::([0-9]+))?((\/\w+)*\/)?([\w\-\.]*)?([#?\w\=]+)?([\&\w=\w]+.*)?([\w\+\-\/\%]*)?[A-Za-z0-9_\/]/g;
  var arr = str.split(/[\n,' ']+/gm);
  // var arr = str.replace(/\n/g, ' ');
  console.log(arr);
